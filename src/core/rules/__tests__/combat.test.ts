...
describe('Combat Rules Engine', () => {
  describe('calculateHitDifficulty', () => {
    ...
    it('applies all modifiers: Building, Smoke, Hull Down, Rear Arc', () => {
      ...
      const target = {
        coord: { q: 3, r: 0 },
        facing: 0 as const, // Target facing same direction -> attacker is behind target!
        size: 4,
        hasSmoke: true,
        isHullDown: true,
      };
      ...
      expect(breakdown.rearArcModifier).toBe(0); // Attacker facing 0, target at (3,0) -> target in front of attacker
    ...
    it('applies rear arc modifier when shooting at a target behind the attacker', () => {
      const board = createTestBoard();
      const attacker = { coord: { q: 0, r: 0 }, facing: 0 as const }; // Attacker facing East
      const target = {
        coord: { q: -3, r: 0 }, // Target is to the West (behind attacker)
        facing: 0 as const,
        size: 4,
        hasSmoke: false,
        isHullDown: false,
  ...
  describe('resolveDamageCheck', () => {
    ...
    it('resolves DAMAGED when attack equals or slightly exceeds armor value', () => {
      // PEN 7 + Roll 3 = 10 vs Armor 10 -> DAMAGED
    ...
    it('resolves DESTROYED when attack exceeds armor value by at least 3', () => {
      // PEN 7 + Roll 6 = 13 vs Armor 10 -> DESTROYED
